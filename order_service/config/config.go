...
func Load() Config {
	...
	config.PostgresUser = cast.ToString(getOrReturnDefaultValue("POSTGRES_USER", "user"))
	config.PostgresPassword = cast.ToString(getOrReturnDefaultValue("POSTGRES_PASSWORD", "mypass"))
